...
#define EXPORT
  ...
  */
  ...
  typedef enum env_keyNumber_ {
    ...
    ENV_DIM,                              /* Stern (Sonne) */
    ENV_WHITE_BLACK,            /* Kreis, halb weiá halb schwarz (Kontrast) */
    ENV_UD,                               /* UD */
  ...
  typedef struct env_keyboardstate_{
    ...
    BYTE          dwKeys[MAX_NUM_KEYS];    /* true if pressed */
  ...
  */
  ...
  ENV_Status_t WINAPI
  ENV_getBrightnessRelative     (ENV_Sensors_t FAR    * const lpstBrightness);
  ...
  ENV_Status_t WINAPI
  ENV_getVentilatorState        (const BYTE               bIndex,
  ...
  ENV_Status_t WINAPI
  ENV_activateBeeper            (const BYTE               bIndex,
                                 const BOOL               bOn);

  ENV_Status_t WINAPI
  ENV_activateLED               (const BYTE               bIndex,
                                 const BOOL               bOn);

  ENV_Status_t WINAPI
  ENV_synchronizeLEDToBeeper    (const BYTE               bIndLED,
                                 const BYTE               bIndBeeper);

  ENV_Status_t WINAPI
  ENV_synchronizeBeeperToLED    (const BYTE               bIndBeeper,
                                 const BYTE               bIndLED);

  ENV_Status_t WINAPI
  ENV_setBeeper                 (const BYTE               bIndex,
                                 const ENV_BeeperState_t FAR    * const lpstState);

  ENV_Status_t WINAPI
  ENV_setLED                    (const BYTE               bIndex,
                                 const ENV_BeeperState_t FAR    * const lpstState);

  ENV_Status_t WINAPI
  ENV_getBeeperState            (const BYTE               bIndex,
                                 ENV_BeeperState_t FAR    * const lpstState);

  ENV_Status_t WINAPI
  ENV_setSchnarre               (const BYTE onoff);

  ENV_Status_t WINAPI
  ENV_writeSerialNumber         (long lSerNo);

  ENV_Status_t WINAPI
  ENV_getLEDState               (const BYTE               bIndex,
                                 ENV_BeeperState_t FAR    * const lpstState);
